...
public:
    ...
    static std::shared_ptr<CNullLog> CreateInstance() { return std::make_shared<CNullLog>(); }
};
...
private:
    ...
    static std::vector<CZIChecks> BitmaskToChecks(uint64_t bitmask)
    {
        ...
        if (bitmask & CZICHECK_HAS_VALID_SUBBLOCK_SEGMENTS) checks.push_back(CZIChecks::SubBlockDirectorySegmentValid);
        ...
        if (bitmask & CZICHECK_DOES_NOT_USE_BINDEX) checks.push_back(CZIChecks::BenabledDocument);
        if (bitmask & CZICHECK_HAS_ONLY_ONE_PIXELTYPE_PER_CHANNEL) checks.push_back(CZIChecks::SamePixeltypePerChannel);
        if (bitmask & CZICHECK_HAS_PLANE_INDICES_STARTING_AT_ZERO)
            checks.push_back(CZIChecks::PlanesIndicesStartAtZero);
        if (bitmask & CZICHECK_HAS_CONSECUTIVE_PLANE_INDICES) checks.push_back(CZIChecks::PlaneIndicesAreConsecutive);
        if (bitmask & CZICHECK_ALL_SUBBLOCKS_HAVE_MINDEX) checks.push_back(CZIChecks::SubblocksHaveMindex);
        if (bitmask & CZICHECK_HAS_BASICALLY_VALID_METADATA) checks.push_back(CZIChecks::BasicMetadataValidation);
        ...
        if (bitmask & CZICHECK_HAS_VALID_SUBBLOCK_BITMAPS) checks.push_back(CZIChecks::CheckSubBlockBitmapValid);
#if FUTURE_CHECKS
        if (bitmask & CZICHECK_HAS_CONSISTENT_MINDICES) checks.push_back(CZIChecks::ConsistentMIndex);
...
public:
    ...
    int Validate(const char* input_path, std::string& json_result, std::string& error_message)
    {
        ...
        try
        {
            ...
            {
                auto checker =
                    CCheckerFactory::CreateChecker(checkType, spReader, *resultsGatherer, checkerAdditionalInfo);
                if (checker != nullptr)
                {
                    checker->RunCheck();
                }
...
extern "C" CAPI_EXPORT int ValidateFile(void* validator, const char* input_path, char* json_buffer,
                                        uint64_t* json_buffer_size, char* error_message, uint64_t* error_message_length)
{
    ...
    if (result != 0)
    {
        ...
        {
            uint64_t copy_len =
                error_msg.length() < (*error_message_length - 1) ? error_msg.length() : (*error_message_length - 1);
