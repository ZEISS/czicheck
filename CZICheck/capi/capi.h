...
#define CZICHECK_ALL_DEFAULT \
    (CZICHECK_ALL & ~CZICHECK_HAS_XML_SCHEMA_VALID_METADATA & ~CZICHECK_HAS_VALID_SUBBLOCK_BITMAPS)
...
///             Return value 4: One or more requested checks are not available (error details in error_message if
/// provided).
extern "C" CAPI_EXPORT int ValidateFile(void* validator, const char* input_path, char* json_buffer,
                                        uint64_t* json_buffer_size, char* error_message,
                                        uint64_t* error_message_length);
...
/// \param size     Pointer to a uint64 which on input contains the size of the buffer, and on output (with return value
/// false) the required size of the buffer.
///
